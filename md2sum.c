...
/* Helper to print out the final hash */
...
/* Hashes a given file stream and formats the output */
static void hash_stream(FILE *file, const char *name) {
  MD2_CTX context;
  int len;
...
int main(int argc, char *argv[]) {
  ...
  /* Digest standard input if no arguments */
  if (argc == 1) {
    hash_stream(stdin, "-");
  } else {
    ...
    for (i = 1; i < argc; i++) {
      if (argv[i][0] == '-' && argv[i][1] == '\0') {
        ...
        if (file == NULL) {
          fprintf(stderr, "md2sum: %s: No such file or directory\n", argv[i]);
          continue;
        }
        hash_stream(file, argv[i]);
        fclose(file);
      }
    }
  }
